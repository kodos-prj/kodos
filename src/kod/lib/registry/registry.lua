...
function registry.validate_program_def(program_def)
...
--- Get merged schema from program (handles allOf inheritance)
-- @param program_def table - program definition
-- @return table merged schema
local function _get_merged_schema(program_def)
    if not program_def then
        return {}
    end
    
    local schema = program_def.schema or {}
    
    -- If no inheritance, return schema as-is
    if not program_def._parent_schema then
        return schema
    end
    
    -- Handle allOf (merged schema from inheritance)
    return registry.merge_schemas(program_def._parent_schema, schema)
end

--- Validate that a value matches expected type
-- @param value any - value to check
-- @param expected_type string - type name (string, integer, number, boolean, array, object)
-- @return boolean true if type matches
local function _check_type(value, expected_type)
    if expected_type == "string" then return type(value) == "string" end
    if expected_type == "integer" then return type(value) == "number" and value % 1 == 0 end
    if expected_type == "number" then return type(value) == "number" end
    if expected_type == "boolean" then return type(value) == "boolean" end
    if expected_type == "array" then return type(value) == "table" end
    if expected_type == "object" then return type(value) == "table" end
    return true  -- Unknown type, assume valid
end

--- Validate config options against schema
-- @param program_def table - program definition
-- @param options table - user-provided config options
-- @return nil, nil on success; nil, error_msg on failure
function registry.validate_config(program_def, options)
    if type(options) ~= "table" then
        return nil, "Options must be a table"
    end
    
    local schema = _get_merged_schema(program_def)
    
    -- Handle allOf (merged schema from inheritance)
    if schema.allOf then
        for _, sub_schema in ipairs(schema.allOf) do
            local ok, err = registry.validate_config(
                {schema = sub_schema},
                options
            )
            if not ok then
    ...
    local properties = schema.properties or schema
    
    for field_name, field_schema in pairs(properties) do
        -- Skip internal fields
        if not field_name:match("^%$") and field_name ~= "properties" and 
           field_name ~= "allOf" and field_name ~= "type" then
            
            if type(field_schema) == "table" then
                local is_required = field_schema.required or false
                
                if options[field_name] == nil then
                    if is_required then
                        return nil, "Required field '" .. field_name .. "' is missing"
                    end
                else
                    -- Field present - check type if specified
                    local expected_type = field_schema.type
                    if expected_type and not _check_type(options[field_name], expected_type) then
                        return nil, "Field '" .. field_name .. "' has wrong type (expected " .. 
                               expected_type .. ", got " .. type(options[field_name]) .. ")"
                    end
                end
            end
        end
    ...
    if program_def.validate and type(program_def.validate) == "function" then
        local ok, err = pcall(program_def.validate, program_def, options)
        if not ok then
            return nil, "Validation hook failed: " .. tostring(err)
        end
    end
