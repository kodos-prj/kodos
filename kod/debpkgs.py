# Deb package based linux (e.g., debian) related functions
...
def generate_index(c, source, repo):
    # http://ftp.ca.debian.org/debian/dists/stable/main/binary-amd64/Packages.gz
    repo_url = f"{source.url}/{repo}/binary-{source.arch}/"
    db_filename = "Packages.gz"
    print(f"Downloading {repo_url}/{db_filename}")
    target = "tmp/index"
    # c.run(f"mkdir -p {target}")
    c.run(f"curl -# {repo_url}{db_filename} -o tmp/{db_filename}")
    c.run(f"zcat tmp/{db_filename} > {target}")

    # packages_desc = glob.glob(f"{target}/*")
    catalog = {}
    with open(target) as fdesc:
        line = fdesc.readline()
        pkginfo = {}
        while line:
            if line[0] != " ":
